    ...
    run: async (client, message) => {
        ...
        if (message.embeds.length > 0) {
            ...
            if (embed.description && embed.description.includes("Bump done!")) {
                ...
                let guildData = await Guilds.findOne({ _id: message.guild.id }) || new Guilds({ _id: message.guild.id, lastBumpUser: null, lastBumpTime: null });

                const pointsToAdd = generatePoints()
